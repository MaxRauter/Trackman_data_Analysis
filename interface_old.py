...
class TrackManGUI(tk.Tk):
    ...
    def setup_analysis_tab(self):
        ...
        # --- Controls Frame (top, fixed height) ---
        controls_frame = ttk.Frame(main_vsplit)
        controls_frame.pack(fill=tk.X, pady=(0, 10))

        # User selection
        ttk.Label(controls_frame, text="User:").pack(side=tk.LEFT, padx=5)
        self.user_var = tk.StringVar()
        self.user_combo = ttk.Combobox(controls_frame, textvariable=self.user_var, state="readonly", width=15)
        self.user_combo.pack(side=tk.LEFT, padx=5)
        self.user_combo.bind("<<ComboboxSelected>>", self.on_user_selected)

        refresh_btn = ttk.Button(controls_frame, text="⟳", width=3, command=self.refresh_analysis_data)
        refresh_btn.pack(side=tk.LEFT, padx=5)

        # Ball type
        ttk.Label(controls_frame, text="Ball Type:").pack(side=tk.LEFT, padx=5)
        self.analysis_ball_type_var = tk.StringVar(value="range")
        ball_type_combo = ttk.Combobox(controls_frame, textvariable=self.analysis_ball_type_var,
                                       values=["premium", "range"], width=10, state="readonly")
        ball_type_combo.pack(side=tk.LEFT, padx=5)
        ball_type_combo.bind("<<ComboboxSelected>>", self.on_ball_type_selected)

        # Comparison mode
        ttk.Label(controls_frame, text="Comparison:").pack(side=tk.LEFT, padx=5)
        self.comparison_mode_var = tk.StringVar(value="clubs")
        self.club_mode_rb = ttk.Radiobutton(controls_frame, text="Multiple Clubs", 
        ...
        self.time_mode_rb = ttk.Radiobutton(controls_frame, text="Club Over Time", 
        ...
        gaussian_rb = ttk.Radiobutton(controls_frame, text="Gaussian", 
                                      variable=self.plot_type_var, value="gaussian")
        gaussian_rb.pack(side=tk.LEFT, padx=5)
        histogram_rb = ttk.Radiobutton(controls_frame, text="Histogram", 
                                       variable=self.plot_type_var, value="histogram")
        histogram_rb.pack(side=tk.LEFT, padx=5)

        # Plot and Save buttons
        self.plot_btn = ttk.Button(controls_frame, text="Generate Plot", command=self.generate_plot)
        self.plot_btn.pack(side=tk.LEFT, padx=5)
        self.plot_btn.config(state="disabled")
        self.save_plot_btn = ttk.Button(controls_frame, text="Save Plot", command=self.save_plot)
    ...
    def on_continue_sessions(self):
        """After session selection, show clubs from those sessions and enable club selection."""
        ...
        if comparison_mode == "time":
            ...
            # Only one session in clubs mode
            self.selected_session_for_clubs = selected_sessions[0] if selected_sessions else None
        ...
        if comparison_mode == "time":
            ...
            df = df[df['Session ID'] == self.selected_session_for_clubs]
    ...
    def on_comparison_mode_changed(self):
        ...
        if comparison_mode == "time":
            ...
            self.sessions_listbox.config(selectmode=tk.SINGLE)
            self.clubs_listbox.config(selectmode=tk.MULTIPLE, state="normal")
            self.clubs_frame.configure(text="Select Clubs")
            self.sessions_frame.configure(text="Select a Session")
            self.plot_btn.config(state="disabled")
            self.sessions_listbox.bind("<<ListboxSelect>>", self.on_sessions_or_clubs_selected)
            self.clubs_listbox.bind("<<ListboxSelect>>", self.on_sessions_or_clubs_selected)
        self.load_data_for_analysis()
    ...
    def on_comparison_mode_changed(self):
        ...
        if comparison_mode == "time":
            ...
            self.clubs_frame.configure(text="Select Clubs")
            self.sessions_frame.configure(text="Select a Session and Continue")
            self.sessions_listbox.bind("<<ListboxSelect>>", self.on_sessions_selected)
            self.clubs_listbox.bind("<<ListboxSelect>>", self.on_club_selected)
        self.load_data_for_analysis()
    ...
    def generate_plot(self):
        ...
        self.current_figure = plt.figure(figsize=(12, 8), dpi=120)
        ...
        if comparison_mode == "time":
            ...
            # ... after plotting ...
            plt.xlabel('Carry Distance (m)', fontsize=16)
            plt.ylabel(ylabel, fontsize=16)
            plt.title(title, fontsize=18)
            plt.tick_params(axis='both', which='major', labelsize=14)
            plt.grid(True, alpha=0.3)
            plt.legend(fontsize=12, loc='upper left', bbox_to_anchor=(1, 1), borderaxespad=1)
            plt.xlim(0, 200)
            ...
            session_id = getattr(self, "selected_session_for_clubs", None)
            if not session_id:
                messagebox.showwarning("Selection Required", "Please select a session and click Continue.")
                return
            ...
            # Filter data to only include the selected session
            df = df[df['Session ID'] == session_id]

            # Prepare plot filename
            clubs_str = "_".join(selected_clubs)
            self.current_plot_filename = f'{username}_club_carry_{plot_type}_{session_id.replace(" ", "_")}_{clubs_str}_{ball_type}.png'

            # Plot for each selected club
            colors = plt.cm.tab20.colors
            stats = {}
            ...
            if plot_type == 'gaussian':
                title = f'Carry Distance Distributions by Club ({username}, {ball_type}, {session_id})'
                ylabel = 'Normalized Probability Density'
                plt.ylim(-0.02 * len(selected_clubs) - 0.05, 1.1)
            else:
                title = f'Carry Distance Histograms by Club ({username}, {ball_type}, {session_id})'
                ylabel = 'Number of Shots'
    ...
    def update_activity_list(self):
        ...
        for i, activity in enumerate(self.activities):
            ...
            self.activity_tree.insert("", tk.END, values=(i+1, date_str, activity_type))
            
        self.log(f"Found {len(self.activities)} range practice activities")
